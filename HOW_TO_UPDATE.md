...
## New Third Party Dependency (Linux-only)

1. Add a new subdirectory within `thirdparty-linux/` containing a tarball and `build-via-container.sh`
1. Update `README.md`
1. Test the build locally by running `cd thirdparty-linux ; mkdir -p out/{bin,lib,include} ; bash NEWTOOL/build-via-container.sh` and check that `out/` contains what you want it to. Iterate to satisfaction.
1. Commit and push.
